...
def _shell(*, body: str, headings: tuple[tuple[int, str, str], ...], page: Page, config, current_route: str, release: str, version_options: str, banner: str | None) -> str:
...
def build_site(*, root: Path, output: Path, release: str, versions: tuple[tuple[str, str], ...] = ()) -> BuildResult:
    ...
    for page in config.pages:
        ...
        path.write_text(_shell(body=rendered.html, headings=rendered.headings, page=page, config=config, current_route=current_route, release=release, version_options=version_options, banner=page_banner), encoding="utf-8")
